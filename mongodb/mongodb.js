...
module.exports = function(RED) {
  ...
  RED.nodes.registerType("mongodb3", function MongoConfigNode(n) {
    ...
    if (this.credentials.user || this.credentials.password) {
      let uriAppend = '', uriarr = this.uri.split(`://`);      
      for(let i=1;i<uriarr.length;i++){
        uriAppend+=uriarr[i] + ((i<(uriarr.length-1))?"://":"") ;
      }
      this.uri = `${uriarr[0]}://${encodeURIComponent(this.credentials.user)}:${encodeURIComponent(this.credentials.password)}@${uriAppend}`;
    }
    node.log(this.uri);
    node.log(`node.context().global.get('uri'):\t${node.context().global.get('uri')};`)/**/
    this.uri = node.context().global.get('uri');
    this.name = n.name;
